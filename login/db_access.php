<?php
    ...
    $stmt = $mysqli->prepare('SELECT id, name, password FROM `user` WHERE email = ? LIMIT 1'); // finds email, limited to one record only
    ...
    if ($stmt->num_rows === 1) { // checks if a match has been found
        $stmt->bind_result($dbid, $dbname, $dbpass); // dbpass holds password column
